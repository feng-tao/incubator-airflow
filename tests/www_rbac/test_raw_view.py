...
class TestMountPoint(unittest.TestCase):
    def setUp(self):
        ...
        app = application.cached_app(config=config, session=Session, testing=True)
...
class TestConfigurationView(TestBase):
    def test_configuration(self):
        resp = self.client.get('configuration', data=dict(
            username='test',
            password='test'
        ), follow_redirects=True)
        self.check_content_in_response(
...
class TestGraphView(TestBase):
    ...
    def test_dt_nr_dr_form_default_parameters(self):
        a = self.appbuilder.sm.find_user(username='test')
        print ('=====', a)
...
class TestAirflowBaseViews(TestBase):
    ...
    def test_index(self):
        resp = self.client.get('/', data=dict(
            username='test',
            password='test'
        ), follow_redirects=True)
    ...
    def test_health(self):
        resp = self.client.get('health', data=dict(
            username='test',
            password='test'
        ), follow_redirects=True)
    ...
    def test_home(self):
        resp = self.client.get('home', data=dict(
            username='test',
            password='test'
        ), follow_redirects=True)
        self.check_content_in_response('DAGs', resp)
    ...
    def test_task(self):
        ...
        resp = self.client.get(url, data=dict(
            username='test',
            password='test'
        ), follow_redirects=True)
    ...
    def test_dag_stats(self):
        resp = self.client.get('dag_stats', data=dict(
            username='test',
            password='test'
        ), follow_redirects=True)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_task_stats(self):
        resp = self.client.get('task_stats', data=dict(
            username='test',
            password='test'
        ), follow_redirects=True)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_refresh(self):
        resp = self.client.get('refresh?dag_id=example_bash_operator', data=dict(
            username='test',
            password='test'
        ), )
        self.check_content_in_response('', resp, resp_code=302)
